...
class Lure(Object):
    ...
    def using(self, time):
        if self.amount > 0:
            timer = 6 + (self.useTime - time) // 1000
            if timer == 0:
                self.amount -= 1
...
class SellFish:
    ...
    def sell(self, inventory: Inventory, mouse, click):
...
class BuyObj:
    ...
    def buy(self, inventory: Inventory, mouse, click):
        overlay = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 100))
        screen.blit(overlay, (self.position.x - 10, self.position.y - 10))
        if (self.position.x < mouse[0] < self.position.x + self.width) and (self.position.y < mouse[1] < self.position.y + self.height):
    ...
    def display(self, screen: pygame.Surface, inventory, rod, boat):
        if self.is_open:
            ...
            for i, fish in enumerate(inventory.fishlist):
                ...
                sellFish.sell(inventory, mouse, click)
                fish.display(screen, position, 90)
                position.x += 120
                if position.x >= SCREEN_WIDTH - 200:
                    position.y += 100
    ...
    def display(self, screen: pygame.Surface, inventory, rod, boat):
            if self.is_open:
                ...
                for i in range(1,3):
                    ...
                    buyObj.buy(inventory, mouse, click)
                    position.y += 40
                    obj.display(screen, position, 80)
                    position.y = 400
                # продажа рыбы
                ...
                buyUpdates.buy(inventory, mouse, click)
                position.x += 100
                boat.display_lvl(screen, position)
                # улучшить удочку
                position.x = 745
...
def main():
    ...
    while running:
        ...
        if not(is_open): # если ничего не открыли, можно дышать и двигаться
            ...
            lure.using(pygame.time.get_ticks())
        # рисуем
        screen.fill(SEACOLOR)
